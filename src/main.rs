...
async fn transmitter(
    ...
    loop {
        ...
        println!("Waiting for a frame to transmit...");
        Timer::after_secs(2).await;
        // create a twai frame for testing purposes

        let frame2 = <EspTwaiFrame as Frame>::new(
            Id::Extended(ExtendedId::new(0x1FFF_FFFF).unwrap()),
            &[0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08],
        )
        .unwrap();

        let result = tx.transmit_async(&frame2).await;
...
async fn main(spawner: Spawner) {
    ...
    // The speed of the CAN bus. => Isabellenhütte defautl bitrate = baudrate = 500.000 bit/s
